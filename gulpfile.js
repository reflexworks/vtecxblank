      ...
      rules: [
        ...
        },
        {
          ...
          use: {
            ...
            options: { emitWarning: true, fix: true, failOnError: true }
          }
        }
      ]
    },
...
gulp.task('watch:components', function() {
  gulp.watch('./src/components/*').on('change', function(changedFile) {
...
gulp.task('watch:sass', function() {
  gulp.watch(['./src/styles/**/*.sass', './src/styles/*.css']).on('change', function(changedFile) {
    let srcfile = changedFile
    if (argv.F) {
      srcfile = './src/components/' + argv.F
      gulp
...
gulp.task('watch:html', function() {
  gulp.watch('./src/*.html').on('change', function(changedFile) {
    ...
    gulp
      ...
      .on('end', function() {
        if (argv.k) {
          const filename =
            'dist/' + changedFile.replace(/^.*[\\\/]/, '').match(/(.*)(?:\.([^.]+$))/)[1] + '.html'
          sendcontent(filename)
        }
      })
  })
...
gulp.task('watch:settings', function() {
  gulp.watch('./setup/_settings/*').on('change', function(changedFile) {
...
gulp.task('copy:pdf', function() {
  return gulp.src(['src/pdf/**'], { base: 'src' }).pipe(gulp.dest('dist'))
})

gulp.task('copy:xls', function() {
  return gulp.src(['src/xls/**'], { base: 'src' }).pipe(gulp.dest('dist'))
})

gulp.task('symlink', function() {
  vfs.src('dist/components', { followSymlinks: false }).pipe(vfs.symlink('test'))
  vfs.src('dist/server', { followSymlinks: false }).pipe(vfs.symlink('test'))
})

gulp.task(
  'build:html_components',
  gulp.series(gulp.parallel('copy:images', 'copy:pdf', 'copy:xls'), function(done) {
    gulp
      .src('./src/*.html')
      .pipe(minifyHtml({ empty: true }))
      .pipe(gulp.dest('./dist'))
...
gulp.task('upload:entry', function(done) {
  recursive('setup', [], function(err, files) {
    files.map(file => {
      if (file.indexOf('bigquery.json') >= 0) {
        sendfile(file, '?_content', false, false, '/d')
      } else if (file.indexOf('template.xml') < 0 && file.indexOf('folderacls.xml') < 0) {
        sendfile(file, '')
      }
    })
    done()
...
gulp.task('upload:counts', function(done) {
  const path =
    argv.t.substr(argv.t.length - 1) === '/' ? argv.t.substr(0, argv.t.length - 1) : argv.t
  var options = {
    method: 'GET',
...
gulp.task('watch:server', function() {
  gulp.watch('./src/server/*').on('change', function(changedFile) {
    let srcfile = changedFile
    if (argv.F) {
    ...
    gulp
      .src(srcfile)
      .pipe(webpackStream(webpackconfig(srcfile.replace(/^.*[\\\/]/, ''), false, false), webpack))
      .on('error', gutil.log)
...
gulp.task('build:server_dist', function(done) {
  gulp.src('./src/server/*').pipe(webpack_files('./src/server', './dist/server', false, done))
})

gulp.task('build:server_test', function(done) {
  gulp.src('./test/*.html').pipe(webpack_files('./src/server', './test/server', false, done))
})

gulp.task(
  'watch',
  gulp.series(
    gulp.parallel('watch:components', 'watch:html', 'watch:settings', 'watch:sass', 'watch:server')
  )
)
...
gulp.task(
  'serve:test',
  gulp.series(
    gulp.parallel('symlink', 'watch:server', function() {
      return serve('test')
    })
  )
)
...
gulp.task('clean-dist-server', function() {
  return gulp.src(['dist/server'], { read: false }).pipe(clean())
})

gulp.task(
  'build',
  gulp.series('clean-dist', 'build:html_components', 'build:server_dist', 'build:server_test')
)
...
gulp.task(
  'deploy',
  gulp.series('clean-dist', 'build:html_components', 'build:server_dist', 'upload')
)

gulp.task('deploy:server', gulp.series('clean-dist-server', 'build:server_dist', 'upload:server'))
gulp.task(
  'build:server',
  gulp.series('clean-dist-server', gulp.parallel('build:server_dist', 'build:server_test'))
)
