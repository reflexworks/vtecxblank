...
import {shipment_class,customer_code,shipment_service_code,getChargeByZone,getPrefecture,getFullDate,getKey } from './put-billing'
...
function getBillingData(entry) {
	...
	const charge_by_zone = getChargeByZone( customer_all,'',prefecture, entry.billing.shipper_code,entry.billing.billing_item,entry.billing.delivery_area)
	...
	const billing_data = {
		...
		'link': [
			{ '___rel': 'self' , '___href': '/billing_data/' + getKey(entry.billing.shipping_date, shipment_service_code,entry.billing.tracking_number) }
		]
	}
