					...
					<div className="vtecx-demo">
						...
						<div className="vtecx-demo-container">
							<div className="vtecx-demo-content">
								...
								<div className="block">
									<div>
										vte.cxではページング機能をコンポーネントで提供しています。<br />
										<code>
											{'import VtecxPagination from \'./vtecx-pagination\''}<br /><br />
											{'<VtecxPagination'}<br />
											{'　　url="/d/sample_list?f"'}<br />
											{'　　selectPage={(_url) => this.getSampleList(_url)}'}<br />
											{'　　maxDisplayRows={50}'}<br />
											{'　　maxButtons={5}'}<br />
											{'　　reload={this.isReload}'}<br />
											{'/>'}
										</code><br />
									</div>
									<div>　　<b>url</b>：一覧取得先のURL。?fパラメータを必ず指定してください。</div>
									<div>　　<b>selectPage</b>：ページ選択した際に実行する関数を指定する(function形式)。_urlに指定したページ情報が追加された一覧取得先のURLが返却されます。</div>
									<div>　　<b>maxDisplayRows</b>：1ページ辺りの最大表示件数</div>
									<div>　　<b>maxButtons</b>：ページ数表示ボタンの最大表示数。5の場合、「{'<<'}{'<'}1,2,3,4,5{'>'}{'>>'}」となる</div>
									<div>　　<b>reload</b>：一覧を再表示するためのフラグです。カーソル(pageindex)を貼り直します。true or false</div>
									<br />
									<br />
									...
									<code>
										/d/sample_list?f&l=50&_pagination=50<br />
										/d/sample_list?f&l=50&n=1<br />
										/d/sample_list?f&c&l=*
									</code>
									<br />
									<hr />
									<b>1番目のリクエスト</b>はページネーションのためにカーソル(pageindex)を作成するリクエストです。<br /><br />
									<code>
									...
									<div>パラメータ説明</div>
									<br />
									<div>　　<b>f</b>：一覧取得リクエスト</div>
									<div>　　<b>l=50</b>：1ページ辺りの最大取得件数</div>
									<div>　　<b>_pagination=50</b>：一覧のカーソル(pageindex)作成リクエスト</div><br />
									これを行うことで「<b>1ページ最大50件のページネーションを50ページまでは行える</b>」ようになります。<br />
									大量のカーソル一覧(pageindex)を張るのは時間がかかるため、初回は50ページまでとし、50ページ以降のカーソルは実際に51ページ以降の表示リクエストが来た段階で以下のリクエストをコンポーネントが実行します。<br />
									<br />
									50ページ以降のカーソル作成リクエスト
									<code>
									...
									<hr />
									<b>2番目のリクエスト</b>は実際に指定ページの一覧を取得するリクエストです。<br /><br />
									<code>
									...
									<div>　　<b>f</b>：一覧取得リクエスト</div>
									<div>　　<b>l=50</b>：1ページ辺りの最大取得件数</div>
									<div>　　<b>n=1</b>：1ページ目の一覧データ取得リクエスト</div><br />
									これを行うことで「<b>1ページ最大50件のページネーションの1ページ目を取得する</b>」ことができます。<br />
									「n」の数値を変更することで指定のページが取得できます。<br />
									<br />
									例えば10ページ目の取得リクエスト
									<code>
										/d/sample_list?f&l=50&n=10
									</code>
									...
									これは「まだそのページのカーソル(pageindex)が作成されていません。」というメッセージとなります。<br />
									このエラーは度々起こります。
									...
									<br />
									<br />
									これを回避するにはカーソルが作成されるまで待機する必要がありますが、<br />
									チュートリアルのサンプルソースでは<b>リトライ実行</b>することで解決しています。<br />
									実際にどのような処理を行っているのかはサンプルソース(/src/components/tutorial_pagination.tsx)を参照して下さい。「<b>1秒間隔で30回リトライする</b>」処理が実行されるようになっています。
									<br />
									<br />

									<hr />
									<b>3番目のリクエスト</b>は一覧の総件数を取得するリクエストです。<br /><br />
									<code>
										/d/sample_list?f&c&l=*
									</code>
									<br />
									...
									<br />
									<div>　　<b>f</b>：一覧取得リクエスト</div>
									<div>　　<b>c</b>：件数取得リクエスト</div>
									<div>　　<b>l=*</b>：アスタリスクを指定することで1ページ辺りの取得総件数を無制限にする</div><br />
									これを行うことで「<b>1ページ辺り無制限の一覧データの件数を取得する</b>」ことができます。<br />
