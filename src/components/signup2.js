...
class Signup2 extends React.Component {
	...
	componentWillMount() {
		axios({
			...
			this.email = response.data.feed.entry[0].title
			this.id = response.data.feed.entry[0].id.substring(1, response.data.feed.entry[0].id.indexOf(','))

			axios({
				url: '/d/' + this.id + '/group/userinfo?e',
				method: 'get',
				headers: {
					'X-Requested-With': 'XMLHttpRequest'
				}
	...
	handleSubmit(e:InputEvent){
		e.preventDefault()
		this.entry.account = {
			firstname: e.target.name[1].value,
			lastname: e.target.name[0].value,
			tel: e.target.tel.value,
			postcode: e.target.postcode.value,
			prefecture_code: e.target.prefecture_code.value,
		...
		this.entry.link = []
		const link = { ___href: '/' + this.id + '/group/userinfo', ___rel: 'self' }
	...
	render() {
		return (
			<div className="col-md-6 col-md-offset-3 col-sm-12">
				<h2 className="login_form__title">アカウント作成</h2>
							...
							{this.state.isConfirmed ? (
								...
								<Form horizontal onSubmit={(e) => this.handleSubmit(e)}>
									...
									<FormGroup controlId="tel">
										<Col md={12}>
											<ControlLabel>電話番号</ControlLabel>
											<FormControl type="text" placeholder="0123-45-1234" />
										</Col>
									</FormGroup>
									...
									<FormGroup controlId="postcode">
										<Col md={12}>
											<ControlLabel>郵便番号</ControlLabel>
											<FormControl type="text" placeholder="例: 123-1234" />
										</Col>
									</FormGroup>
